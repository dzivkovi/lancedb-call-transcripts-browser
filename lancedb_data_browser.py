...
def load_table_metadata():
    ...
    display_columns = ['session_id', 'timestamp', 'text', 'target', 'session_type', 'content_type']
    ...
    return {
        ...
        'sample': sample,
        'unique_sessions': sample['session_id'].nunique() if 'session_id' in sample.columns else 0
    }
...
def main():
    ...
    # Main content tabs
    tab1, tab2, tab3 = st.tabs(["Raw Data Browser", "Session Transcripts", "Data Export"])
    ...
    with tab1:
        ...
        with col4:
            ...
            st.write(f"Records {(page_num-1)*page_size + 1:,}-{min(page_num*page_size, metadata['total_rows']):,}")
        ...
        # Data display
        if show_columns:
            page_data = get_data_page(table, page_num, page_size, show_columns + (['readable_time'] if 'timestamp' in show_columns else []))
    ...
    with tab3:
        ...
        with export_col1:
            ...
            if st.button("Export Current Page"):
                current_data = get_data_page(table, page_num, page_size, metadata['display_columns'])
            ...
            if st.button("Export All Readable Data"):
                ...
                if 'timestamp' in all_data.columns:
                    all_data['readable_time'] = all_data['timestamp'].apply(convert_timestamp)
    ...
    if search_term:
        with st.spinner("Searching..."):
            ...
            if 'text' in all_data.columns:
                matches = all_data[all_data['text'].str.contains(search_term, case=False, na=False)]
                ...
                if not matches.empty:
                    ...
                    for idx, (_, row) in enumerate(matches.head(5).iterrows()):
                        with st.expander(f"Match {idx + 1}: {row.get('session_id', 'Unknown')}"):
                            text = row['text']
                            # Highlight search term
                            start = text.lower().find(search_term.lower())
                            if start != -1:
                                preview = text[max(0, start-50):start+len(search_term)+50]
                                st.write(f"...{preview}...")
