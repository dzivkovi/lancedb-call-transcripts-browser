...
def get_latest_versions():
    ...
    for package in packages:
        try:
            ...
            # Get latest version
            result = subprocess.run([sys.executable, "-m", "pip", "index", "versions", package], 
                                  capture_output=True, text=True)
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if 'Available versions:' in line:
                        ...
                        versions_line = next((l for l in lines if l.strip() and not l.startswith('Available')), '')
                        if versions_line:
            ...
            print(f"{package:12} | Current: {current_version:12} | Latest: {latest_version}")
