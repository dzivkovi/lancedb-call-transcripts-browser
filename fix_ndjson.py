...
Examples:
  ...
  %(prog)s data/case-name/sessions.ndjson    # Fix investigative data
    ...
    parser.add_argument(
        "-o", "--output", help="Output file (default: input_fixed.ndjson)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dry_run:
        ...
        print(
            f"🔧 Would fix {error_lines} lines and recover ~{error_lines * 2} objects"
        )
        return
    ...
    try:
        ...
        print(
            f"🎯 Success rate: {(objects_recovered / (lines_fixed * 2 + total_lines - lines_fixed)) * 100:.1f}%"
        )
        ...
        if lines_fixed > 0:
            ...
            print("   • Potential evidence preservation: CRITICAL")
